...
/*
 * Terminal Screens
 * The devcon_screen layer implements the terminal-side. It handles all
 * commands returned by the seq-parser and applies them to its own pages.
 *
...
enum {
	...
	DEVCON_FLAG_7BIT_MODE		= (1U << 0),
	/* hide cursor caret (default: off) */
	DEVCON_FLAG_HIDE_CURSOR		= (1U << 1),
	/* do not send TPARM unrequested (default: off) */
	DEVCON_FLAG_INHIBIT_TPARM	= (1U << 2),
	/* perform carriage-return on line-feeds (default: off) */
	DEVCON_FLAG_NEWLINE_MODE	= (1U << 3),
	/* wrap-around is pending */
	DEVCON_FLAG_PENDING_WRAP	= (1U << 4),
	/* application-keypad mode (default: off) */
	DEVCON_FLAG_KEYPAD_MODE		= (1U << 5),
	/* enable application cursor-keys (default: off) */
	DEVCON_FLAG_CURSOR_KEYS		= (1U << 6),
};

enum {
	DEVCON_CONFORMANCE_LEVEL_VT52,
	DEVCON_CONFORMANCE_LEVEL_VT100,
	DEVCON_CONFORMANCE_LEVEL_VT400,
	DEVCON_CONFORMANCE_LEVEL_N,
};
...
struct devcon_screen {
	...
	struct devcon_page *page_main;
	struct devcon_page *page_alt;
	struct devcon_history *history;
	struct devcon_history *history_main;

	unsigned int n_tabs;
	u8 *tabs;

	struct devcon_utf8 utf8;
	...
	struct devcon_attr default_attr;
	...
	struct devcon_state state;
	struct devcon_state saved;
	struct devcon_state saved_alt;
};
		      ...
		      void *cmd_fn_data)
{
	...
	screen->flags = DEVCON_FLAG_7BIT_MODE;
	screen->conformance_level = DEVCON_CONFORMANCE_LEVEL_VT400;
	screen->g0 = &devcon_unicode_lower;
	screen->g1 = &devcon_unicode_upper;
	screen->g2 = &devcon_unicode_lower;
	screen->g3 = &devcon_unicode_upper;
	screen->state.gl = &screen->g0;
	screen->state.gr = &screen->g1;
	screen->saved = screen->state;
	screen->saved_alt = screen->saved;

	ret = devcon_page_new(&screen->page_main);
	if (ret < 0)
		goto error;

	ret = devcon_page_new(&screen->page_alt);
	if (ret < 0)
		goto error;

	ret = devcon_parser_new(&screen->parser);
	if (ret < 0)
		goto error;

	ret = devcon_history_new(&screen->history_main);
	if (ret < 0)
		goto error;
...
struct devcon_screen *devcon_screen_free(struct devcon_screen *screen)
{
	...
	kfree(screen->answerback);
	kfree(screen->tabs);
	devcon_history_free(screen->history_main);
	devcon_page_free(screen->page_alt);
	devcon_page_free(screen->page_main);
	devcon_parser_free(screen->parser);
	kfree(screen);
 ...
 * them out once their call to devcon_screen_feed_*() returns.
...
#define SEQ(_screen, _prefix_esc, _c0, _c1, _seq) \
	(((_screen)->flags & DEVCON_FLAG_7BIT_MODE) ? \
...
static int screen_write(struct devcon_screen *unlocked_screen,
			const void *data,
			size_t size)
{
	if (size < 1 || !unlocked_screen->write_fn)
		return 0;

	return unlocked_screen->write_fn(unlocked_screen,
					 unlocked_screen->write_fn_data,
					 data,
					 size);
}
...
static int screen_forward(struct devcon_screen *screen,
			  unsigned int cmd,
...
static unsigned int screen_clamp_x(struct devcon_screen *screen,
				   unsigned int x)
{
	if (x >= screen->page->width)
		return (screen->page->width > 0) ?
					screen->page->width - 1 : 0;

	return x;
}

static unsigned int screen_clamp_y(struct devcon_screen *screen,
				   unsigned int y)
{
	if (y >= screen->page->height)
		return (screen->page->height > 0) ?
					screen->page->height - 1 : 0;

	return y;
}

static bool screen_tab_is_set(struct devcon_screen *screen, unsigned int pos)
{
...
static inline void screen_age_cursor(struct devcon_screen *screen)
{
	struct devcon_cell *cell;

	cell = devcon_page_get_cell(screen->page,
...
static void screen_cursor_clear_wrap(struct devcon_screen *screen)
{
	screen->flags &= ~DEVCON_FLAG_PENDING_WRAP;
}

static void screen_cursor_set(struct devcon_screen *screen,
			      unsigned int x,
			      unsigned int y)
{
	...
	if (!(screen->flags & DEVCON_FLAG_HIDE_CURSOR))
...
static void screen_cursor_set_rel(struct devcon_screen *screen,
				  unsigned int x,
				  unsigned int y)
{
...
static void screen_cursor_left(struct devcon_screen *screen, unsigned int num)
{
	if (num > screen->state.cursor_x)
		num = screen->state.cursor_x;

	screen_cursor_set(screen,
			  screen->state.cursor_x - num,
			  screen->state.cursor_y);
}

static void screen_cursor_left_tab(struct devcon_screen *screen,
				   unsigned int num)
{
	unsigned int i;
...
static void screen_cursor_right(struct devcon_screen *screen, unsigned int num)
{
...
static void screen_cursor_right_tab(struct devcon_screen *screen,
				    unsigned int num)
{
	unsigned int i;
...
static void screen_cursor_up(struct devcon_screen *screen,
			     unsigned int num,
			     bool scroll)
{
	unsigned int max;
	...
	if (screen->state.cursor_y < screen->page->scroll_idx) {
		...
		if (num > max) {
			...
			if (scroll)
				devcon_page_scroll_down(screen->page,
							num - max,
							&screen->state.attr,
							screen->age,
							NULL);
...
static void screen_cursor_down(struct devcon_screen *screen,
			       unsigned int num,
			       bool scroll)
{
	unsigned int max;
	...
	if (screen->state.cursor_y >= screen->page->scroll_idx +
		...
		max = screen->page->scroll_idx +
		      screen->page->scroll_num - 1 - screen->state.cursor_y;
		if (num > max) {
			if (num < 1)
				return;
			...
			if (scroll)
				devcon_page_scroll_up(screen->page,
						      num - max,
						      &screen->state.attr,
						      screen->age,
						      screen->history);
...
static void screen_save_state(struct devcon_screen *screen,
			      struct devcon_state *where)
{
	*where = screen->state;
}

static void screen_restore_state(struct devcon_screen *screen,
				 const struct devcon_state *from)
{
	screen_cursor_set(screen, from->cursor_x, from->cursor_y);
	screen->state = *from;
}

static void screen_reset_page(struct devcon_screen *screen,
			      struct devcon_page *page)
{
	devcon_page_set_scroll_region(page, 0, page->height);
	devcon_page_erase(page, 0, 0, page->width, page->height,
			  &screen->state.attr, screen->age, false);
}

static void screen_change_alt(struct devcon_screen *screen, bool set)
{
	if (set) {
...
static inline void set_reset(struct devcon_screen *screen,
			     unsigned int flag,
			     bool set)
{
	if (set)
...
static void screen_mode_change_ansi(struct devcon_screen *screen,
				    unsigned mode,
				    bool set)
{
	switch (mode) {
	case 20:
		 ...
		 * TODO
		 */
		set_reset(screen, DEVCON_FLAG_NEWLINE_MODE, set);

		break;
	default:
		pr_debug("failed to %s unknown ANSI mode %u",
			 set ? "set" : "unset", mode);
		break;
	}
}

static void screen_mode_change_dec(struct devcon_screen *screen,
				   unsigned int mode,
				   bool set)
{
	switch (mode) {
	case 1:
		 ...
		 * TODO
		 */
		set_reset(screen, DEVCON_FLAG_CURSOR_KEYS, set);
	...
	case 25:
		 ...
		 * TODO
		 */
		set_reset(screen, DEVCON_FLAG_HIDE_CURSOR, !set);
		screen_age_cursor(screen);
	...
	case 1049:
		...
		break;
	default:
		pr_debug("failed to %s unknown DEC mode %u",
			 set ? "set" : "unset", mode);
		break;
	}
}
...
static u32 screen_map(struct devcon_screen *screen, u32 val)
{
	u32 nval = -1U;

	/*
	 * 32 and 127 always map to identity. 160 and 255 map to identity if a
	 * 94 character set is loaded into GR. Values above 255 always map to
	 * identity.
	 */
	switch (val) {
	case 33 ... 126:
...
/*
 * Command Handlers
 * This is the unofficial documentation of all the DEVCON_CMD_* definitions.
...
static int screen_DA1(struct devcon_screen *screen,
		      const struct devcon_seq *seq);
static int screen_LF(struct devcon_screen *screen,
		     const struct devcon_seq *seq);

static int screen_GRAPHIC(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	struct devcon_char ch = DEVCON_CHAR_NULL;
	...
	ch = devcon_char_merge(ch, screen_map(screen, seq->terminator));
	devcon_page_write(screen->page,
			  screen->state.cursor_x,
			  screen->state.cursor_y,
			  ch,
			  1,
...
static int screen_BEL(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * BEL - sound bell tone
	...
	return screen_forward(screen, DEVCON_CMD_BEL, seq);
}

static int screen_BS(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * BS - backspace
...
static int screen_CBT(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_CHA(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_CHT(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_CNL(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_CPL(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_CR(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * CR - carriage-return
...
static int screen_CUB(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * CUB - cursor-backward
...
static int screen_CUD(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * CUD - cursor-down
...
static int screen_CUF(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * CUF -cursor-forward
...
static int screen_CUP(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * CUP - cursor-position
...
static int screen_CUU(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * CUU - cursor-up
...
static int screen_DA2(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_DA3(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_DC1(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_DC3(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_DCH(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	...
	screen_cursor_clear_wrap(screen);
	devcon_page_delete_cells(screen->page,
				 screen->state.cursor_x,
				 screen->state.cursor_y,
				 num,
				 &screen->state.attr,
...
static int screen_DECALN(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECANM(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
	 * DECANM - ansi-mode
...
static int screen_DECBI(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECBI - back-index
...
static int screen_DECCARA(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECCRA(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECDC(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECDC - delete-column
...
static int screen_DECDHL_BH(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_DECDHL_TH(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_DECDWL(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECEFR(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECELF(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECELR(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECERA(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECFI(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECFI - forward-index
...
static int screen_DECFRA(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECIC(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECIC - insert-column
...
static int screen_DECID(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECID - return-terminal-id
	 * This is an obsolete form of DEVCON_CMD_DA1.
	 */

	return screen_DA1(screen, seq);
}

static int screen_DECINVM(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECKBD(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECKPAM(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
	...
	screen->flags |= DEVCON_FLAG_KEYPAD_MODE;

	return 0;
}

static int screen_DECKPNM(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECLFKC(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECLL(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECLL - load-leds
...
static int screen_DECLTOD(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECPCTERM(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_DECPKA(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECPKFMR(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRARA(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRC(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
...
static int screen_DECREQTPARM(struct devcon_screen *screen,
			      const struct devcon_seq *seq)
{
	/*
	...
	if (seq->n_args < 1 || seq->args[0] == 0) {
		screen->flags &= ~DEVCON_FLAG_INHIBIT_TPARM;
...
static int screen_DECRPKT(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQCRA(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQDE(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQKT(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQLP(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQM_ANSI(struct devcon_screen *screen,
			      const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQM_DEC(struct devcon_screen *screen,
			     const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQPKFM(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQPSR(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQTSR(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECRQUPSS(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSACE(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSASD(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSC(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECSC - save-cursor
...
static int screen_DECSCA(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSCL(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
	...
	devcon_screen_hard_reset(screen);

	switch (level) {
	case 61:
		screen->conformance_level = DEVCON_CONFORMANCE_LEVEL_VT100;
		screen->flags |= DEVCON_FLAG_7BIT_MODE;
		break;
	case 62 ... 69:
		screen->conformance_level = DEVCON_CONFORMANCE_LEVEL_VT400;
...
static int screen_DECSCP(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSCPP(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSCS(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSCUSR(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSDDT(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSDPT(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSED(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSEL(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSERA(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSFC(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSKCV(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSLCK(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSLE(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSLPP(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSLRM_OR_SC(struct devcon_screen *screen,
				const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSMBV(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSMKR(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSNLS(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSPP(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSPPCS(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSPRTT(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSR(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	 * DECSR - secure-reset
...
static int screen_DECSRFR(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSSCLS(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSSDT(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSSL(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECST8C(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSTBM(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
	...
	devcon_page_set_scroll_region(screen->page, top - 1, bottom - top + 1);
	screen_cursor_clear_wrap(screen);
...
static int screen_DECSTR(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
	...
	devcon_screen_soft_reset(screen);

	return 0;
}

static int screen_DECSTRL(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSWBV(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_DECSWL(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECTID(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECTME(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DECTST(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_DL(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * DL - delete-line
	...
	devcon_page_delete_lines(screen->page,
				 screen->state.cursor_y,
				 num,
				 &screen->state.attr,
				 screen->age);
...
static int screen_DSR_ANSI(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_DSR_DEC(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_ECH(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * ECH - erase-character
...
static int screen_ED(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * ED - erase-in-display
...
static int screen_EL(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * EL - erase-in-line
...
static int screen_ENQ(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * ENQ - enquiry
...
static int screen_EPA(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_FF(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * FF - form-feed
...
static int screen_HPA(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_HPR(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_HT(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * HT - horizontal-tab
...
static int screen_HTS(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_HVP(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_ICH(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	...
	screen_cursor_clear_wrap(screen);
	devcon_page_insert_cells(screen->page,
				 screen->state.cursor_x,
				 screen->state.cursor_y,
				 num,
				 &screen->state.attr,
...
static int screen_IL(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * IL - insert-line
	...
	screen_cursor_clear_wrap(screen);
	devcon_page_insert_lines(screen->page,
				 screen->state.cursor_y,
				 num,
				 &screen->state.attr,
				 screen->age);
...
static int screen_IND(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * IND - index
		     ...
		     const struct devcon_seq *seq)
{
	...
	screen_cursor_down(screen, 1, true);
	if (screen->flags & DEVCON_FLAG_NEWLINE_MODE)
		screen_cursor_left(screen, screen->state.cursor_x);

	return 0;
}

static int screen_LS1R(struct devcon_screen *screen,
		       const struct devcon_seq *seq)
{
	/*
...
static int screen_LS2(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * LS2 - locking-shift-2
...
static int screen_LS2R(struct devcon_screen *screen,
		       const struct devcon_seq *seq)
{
	/*
...
static int screen_LS3(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * LS3 - locking-shift-3
...
static int screen_LS3R(struct devcon_screen *screen,
		       const struct devcon_seq *seq)
{
	/*
...
static int screen_MC_ANSI(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_MC_DEC(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_NEL(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * NEL - next-line
...
static int screen_NP(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * NP - next-page
...
static int screen_NULL(struct devcon_screen *screen,
		       const struct devcon_seq *seq)
{
	/*
	 * NULL - null
...
static int screen_PP(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * PP - preceding-page
...
static int screen_PPA(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_PPB(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_PPR(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_RC(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * RC - restore-cursor
...
static int screen_REP(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * REP - repeat
...
static int screen_RI(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * RI - reverse-index
...
static int screen_RIS(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_RM_ANSI(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_RM_DEC(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
...
static int screen_S7C1T(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
	...
	if (screen->conformance_level > DEVCON_CONFORMANCE_LEVEL_VT100)
		screen->flags |= DEVCON_FLAG_7BIT_MODE;

	return 0;
}

static int screen_S8C1T(struct devcon_screen *screen,
			const struct devcon_seq *seq)
{
	/*
...
static int screen_SCS(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	...
	devcon_charset *cs = NULL;

	/* TODO: support more of them? */
	switch (seq->charset) {
	case DEVCON_CHARSET_ISO_LATIN1_SUPPLEMENTAL:
	case DEVCON_CHARSET_ISO_LATIN2_SUPPLEMENTAL:
	case DEVCON_CHARSET_ISO_LATIN5_SUPPLEMENTAL:
	case DEVCON_CHARSET_ISO_GREEK_SUPPLEMENTAL:
	case DEVCON_CHARSET_ISO_HEBREW_SUPPLEMENTAL:
	case DEVCON_CHARSET_ISO_LATIN_CYRILLIC:
		break;

	case DEVCON_CHARSET_DEC_SPECIAL_GRAPHIC:
		cs = &devcon_dec_special_graphics;
		break;
	case DEVCON_CHARSET_DEC_SUPPLEMENTAL:
		cs = &devcon_dec_supplemental_graphics;
		break;
	case DEVCON_CHARSET_DEC_TECHNICAL:
	case DEVCON_CHARSET_CYRILLIC_DEC:
	case DEVCON_CHARSET_DUTCH_NRCS:
	case DEVCON_CHARSET_FINNISH_NRCS:
	case DEVCON_CHARSET_FRENCH_NRCS:
	case DEVCON_CHARSET_FRENCH_CANADIAN_NRCS:
	case DEVCON_CHARSET_GERMAN_NRCS:
	case DEVCON_CHARSET_GREEK_DEC:
	case DEVCON_CHARSET_GREEK_NRCS:
	case DEVCON_CHARSET_HEBREW_DEC:
	case DEVCON_CHARSET_HEBREW_NRCS:
	case DEVCON_CHARSET_ITALIAN_NRCS:
	case DEVCON_CHARSET_NORWEGIAN_DANISH_NRCS:
	case DEVCON_CHARSET_PORTUGUESE_NRCS:
	case DEVCON_CHARSET_RUSSIAN_NRCS:
	case DEVCON_CHARSET_SCS_NRCS:
	case DEVCON_CHARSET_SPANISH_NRCS:
	case DEVCON_CHARSET_SWEDISH_NRCS:
	case DEVCON_CHARSET_SWISS_NRCS:
	case DEVCON_CHARSET_TURKISH_DEC:
	case DEVCON_CHARSET_TURKISH_NRCS:
		break;

	case DEVCON_CHARSET_USERPREF_SUPPLEMENTAL:
		break;
	}

	if (seq->intermediates & DEVCON_SEQ_FLAG_POPEN)
		screen->g0 = cs ? : &devcon_unicode_lower;
	else if (seq->intermediates & DEVCON_SEQ_FLAG_PCLOSE)
		screen->g1 = cs ? : &devcon_unicode_upper;
	else if (seq->intermediates & DEVCON_SEQ_FLAG_MULT)
		screen->g2 = cs ? : &devcon_unicode_lower;
	else if (seq->intermediates & DEVCON_SEQ_FLAG_PLUS)
		screen->g3 = cs ? : &devcon_unicode_upper;
	else if (seq->intermediates & DEVCON_SEQ_FLAG_MINUS)
		screen->g1 = cs ? : &devcon_unicode_upper;
	else if (seq->intermediates & DEVCON_SEQ_FLAG_DOT)
		screen->g2 = cs ? : &devcon_unicode_lower;
	else if (seq->intermediates & DEVCON_SEQ_FLAG_SLASH)
		screen->g3 = cs ? : &devcon_unicode_upper;

	return 0;
}

static int screen_SD(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * SD - scroll-down
...
static int screen_SGR(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_SI(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * SI - shift-in
...
static int screen_SM_ANSI(struct devcon_screen *screen,
			  const struct devcon_seq *seq)
{
	/*
...
static int screen_SM_DEC(struct devcon_screen *screen,
			 const struct devcon_seq *seq)
{
	/*
	 * SM_DEC - set-mode-dec
...
static int screen_SO(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * SO - shift-out
...
static int screen_SPA(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_SS2(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * SS2 - single-shift-2
...
static int screen_SS3(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * SS3 - single-shift-3
...
static int screen_ST(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
...
static int screen_SU(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * SU - scroll-up
...
static int screen_SUB(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * SUB - substitute
	...
	static const struct devcon_seq rep = {
		.type = DEVCON_SEQ_GRAPHIC,
		.command = DEVCON_CMD_GRAPHIC,
		.terminator = 0xfffd,
	};
...
static int screen_TBC(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
	 * TBC - tab-clear
...
static int screen_VPA(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_VPR(struct devcon_screen *screen,
		      const struct devcon_seq *seq)
{
	/*
...
static int screen_VT(struct devcon_screen *screen,
		     const struct devcon_seq *seq)
{
	/*
	 * VT - vertical-tab
...
static int screen_XTERM_CLLHP(struct devcon_screen *screen,
			      const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_IHMT(struct devcon_screen *screen,
			     const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_MLHP(struct devcon_screen *screen,
			     const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_MUHP(struct devcon_screen *screen,
			     const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_RPM(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_RRV(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_RTM(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SACL1(struct devcon_screen *screen,
			      const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SACL2(struct devcon_screen *screen,
			      const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SACL3(struct devcon_screen *screen,
			      const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SDCS(struct devcon_screen *screen,
			     const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SGFX(struct devcon_screen *screen,
			     const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SPM(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SRV(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_STM(struct devcon_screen *screen,
			    const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_SUCS(struct devcon_screen *screen,
			     const struct devcon_seq *seq)
{
	/*
...
static int screen_XTERM_WM(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	/*
...
static int screen_feed_cmd(struct devcon_screen *screen,
			   const struct devcon_seq *seq)
{
	switch (seq->command) {
	case DEVCON_CMD_GRAPHIC:
	...
	case DEVCON_CMD_SM_ANSI:
		return screen_SM_ANSI(screen, seq);
	case DEVCON_CMD_SM_DEC:
			    ...
			    size_t size)
{
	 ...
	 */
	for (i = 0; i < size; ++i) {
		ucs4_len = devcon_utf8_decode(&screen->utf8,
					      &ucs4_str,
					      in[i]);

		for (j = 0; j < ucs4_len; ++j) {
			ret = devcon_parser_feed(screen->parser,
						 &seq,
						 ucs4_str[j]);
			if (ret < 0)
				return ret;
			if (ret != DEVCON_SEQ_NONE) {
				ret = screen_feed_cmd(screen, seq);
				if (ret < 0)
					return ret;
			}
		}
...
static char *screen_map_key(unsigned int flags,
			    char *p,
			    const u32 *keysyms,
			    size_t n_syms,
			    u32 ascii,
			    ...
			    unsigned int mods)
{
	 ...
	 * encoding is rather arbitrary, but shared across all CSI handlers.
	 * Pre-calculate it to avoid calculating it in each handler.
	 */
	ch_mods = 0;
	switch (mods & (DEVCON_MOD_SHIFT | DEVCON_MOD_ALT | DEVCON_MOD_CTRL)) {
	case DEVCON_MOD_SHIFT:
		ch_mods = '2';
		break;
	case DEVCON_MOD_ALT:
		ch_mods = '3';
		break;
	case DEVCON_MOD_SHIFT | DEVCON_MOD_ALT:
		ch_mods = '4';
		break;
	case DEVCON_MOD_CTRL:
		ch_mods = '5';
		break;
	case DEVCON_MOD_CTRL | DEVCON_MOD_SHIFT:
		ch_mods = '6';
		break;
	case DEVCON_MOD_CTRL | DEVCON_MOD_ALT:
		ch_mods = '7';
		break;
	case DEVCON_MOD_CTRL | DEVCON_MOD_SHIFT | DEVCON_MOD_ALT:
		ch_mods = '8';
		break;
	}
	 ...
	 * input. @keysyms contains the actual keysym that the user
	...
	ch = 0;
	switch (v) {
	case KEY_UP: /* XKB_KEY_Up */
		ch = 'A';
		break;
	case KEY_DOWN: /* XKB_KEY_Down */
		ch = 'B';
		break;
	case KEY_RIGHT: /* XKB_KEY_Right */
		ch = 'C';
		break;
	case KEY_LEFT: /* XKB_KEY_Left */
		ch = 'D';
		break;
	case KEY_HOME: /* XKB_KEY_Home */
		ch = 'H';
		break;
	case KEY_END: /* XKB_KEY_End */
		ch = 'F';
		break;
	}
	if (ch) {
		*p++ = 0x1b;
		if (flags & DEVCON_FLAG_CURSOR_KEYS)
			*p++ = 'O';
		else
			*p++ = '[';
		if (ch_mods) {
	...
	ch = 0;
	switch (v) {
	case KEY_FIND: /* XKB_KEY_Find */
		ch = '1';
		break;
	case KEY_INSERT: /* XKB_KEY_Insert */
		ch = '2';
		break;
	case KEY_DELETE: /* XKB_KEY_Delete */
		ch = '3';
		break;
	case KEY_SELECT: /* XKB_KEY_Select */
		ch = '4';
		break;
	case KEY_PAGEUP: /* XKB_KEY_Page_Up */
		ch = '5';
		break;
	case KEY_PAGEDOWN: /* XKB_KEY_Page_Down */
		ch = '6';
		break;
	}
	if (ch) {
	...
	ch = 0;
	switch (v) {
	case KEY_F1: /* XKB_KEY_F1 */
		ch = 'P';
		break;
	case KEY_F2: /* XKB_KEY_F2 */
		ch = 'Q';
		break;
	case KEY_F3: /* XKB_KEY_F3 */
		ch = 'R';
		break;
	case KEY_F4: /* XKB_KEY_F4 */
		ch = 'S';
		break;
	}
	if (ch) {
	...
	ch2 = 0;
	switch (v) {
	case KEY_F5: /* XKB_KEY_F5 */
		ch = '1';
		ch2 = '5';
		break;
	case KEY_F6: /* XKB_KEY_F6 */
		ch = '1';
		ch2 = '7';
		break;
	case KEY_F7: /* XKB_KEY_F7 */
		ch = '1';
		ch2 = '8';
		break;
	case KEY_F8: /* XKB_KEY_F8 */
		ch = '1';
		ch2 = '9';
		break;
	case KEY_F9: /* XKB_KEY_F9 */
		ch = '2';
		ch2 = '0';
		break;
	case KEY_F10: /* XKB_KEY_F10 */
		ch = '2';
		ch2 = '1';
		break;
	case KEY_F11: /* XKB_KEY_F11 */
		ch = '2';
		ch2 = '2';
		break;
	case KEY_F12: /* XKB_KEY_F12 */
		ch = '2';
		ch2 = '3';
		break;
	}
	...
	/* map special keys */
	switch (v) {
	case KEY_BACKSPACE:	/* 0xff08: XKB_KEY_BackSpace */
		/* linux default is DEL, so ^H (0x08) can be distinguished */
		*p++ = 0x7f;
		return p;
	case KEY_TAB:		/* 0xff09: XKB_KEY_Tab, XKB_KEY_ISO_Left_tab */
		*p++ = 0x09;
		return p;
	case KEY_LINEFEED:	/* 0xff0a: XKB_KEY_Linefeed */
		*p++ = 0x0a;
		return p;
	case KEY_CLEAR:		/* 0xff0b: XKB_KEY_Clear */
		*p++ = 0x0b;
		return p;
	case KEY_SYSRQ:		/* 0xff15: XKB_KEY_Sys_Req */
		*p++ = 0x15;
		return p;
	case KEY_ESC:		/* 0xff1b: XKB_KEY_Escape */
		*p++ = 0x1b;
		return p;
	case KEY_DELETE:	/* 0xffff: XKB_KEY_Delete */
		*p++ = 0xff;
		return p;
	case KEY_PAUSE:		/* 0xff13: XKB_KEY_Pause */
		...
		return p;
	case KEY_SCROLLLOCK:	/* 0xff14: XKB_KEY_Scroll_Lock */
		 ...
		 * this, we can re-enable it. */
		return p;
	case KEY_ENTER:		/* XKB_KEY_Return */
		*p++ = 0x0d;
		if (flags & DEVCON_FLAG_NEWLINE_MODE)
			*p++ = 0x0a;
		return p;
	}
	...
	/* map unicode keys */
	for (i = 0; i < n_syms; ++i)
		if (ucs4[i] > 0 && ucs4[i] != -1)
			p += devcon_utf8_encode(p, ucs4[i]);

	return p;
}

int devcon_screen_feed_keyboard(struct devcon_screen *unlocked_screen,
				const u32 *keysyms,
				size_t n_syms,
				u32 ascii,
				const u32 *ucs4,
				unsigned int mods)
{
	...
	/*
	 * We can be called from atomic-context, so there's no way screen is
	 * properly locked. However, that's fine as all we need as context is
	 * the current flags.
	 * There might be races if someone modifies them in parallel, but
	 * they can be safely ignored as we do not guarantee synchronous
	 * behavior in those cases (as it would not make any difference).
	 */
	flags = ACCESS_ONCE(unlocked_screen->flags);

	/* allocate buffer if too small */
	start = buf;
	if (4 * n_syms + padding > sizeof(buf)) {
		dyn = kmalloc(4 * n_syms + padding, GFP_TEMPORARY);
		if (!dyn)
			return -ENOMEM;
	...
	p = screen_map_key(flags, p, keysyms, n_syms, ascii, ucs4, mods);
	if (p && p - start > 0) {
		/*
		 * The ALT modifier causes ESC to be prepended to any
		 * key-stroke. We already accounted for that buffer space
		 * above, so simply prepend it here.
		 *
		 * TODO: is altSendsEscape a suitable default? What are the
		 *       semantics exactly? Is it used in C0/C1 conversion? Is
		 *       it prepended if there already is an escape character?
		 */
		if (mods & DEVCON_MOD_ALT && *start != 0x1b)
			*--start = 0x1b;

		/* turn C0 into C1 */
		if (!(flags & DEVCON_FLAG_7BIT_MODE) && p - start >= 2)
			if (start[0] == 0x1b &&
			    start[1] >= 0x40 &&
			    start[1] <= 0x5f)
				*++start ^= 0x40;

		ret = screen_write(unlocked_screen, start, p - start);
	}
			 ...
			 unsigned int y)
{
	...
	ret = devcon_page_reserve(screen->page_main, x, y,
				  &screen->state.attr, screen->age);
	if (ret < 0)
		return ret;

	ret = devcon_page_reserve(screen->page_alt, x, y,
				  &screen->state.attr, screen->age);
	if (ret < 0)
		return ret;

	if (x > screen->n_tabs) {
		t = krealloc(screen->tabs, (x + 7) / 8, GFP_KERNEL);
		if (!t)
			return -ENOMEM;
	...
	devcon_page_resize(screen->page_main, x, y, &screen->state.attr,
			   screen->age, screen->history);
	devcon_page_resize(screen->page_alt, x, y, &screen->state.attr,
			   screen->age, NULL);
...
void devcon_screen_soft_reset(struct devcon_screen *screen)
{
	...
	devcon_page_set_scroll_region(screen->page_main, 0,
				      screen->page->height);
	devcon_page_set_scroll_region(screen->page_alt, 0,
				      screen->page->height);
}
...
{
	devcon_screen_soft_reset(screen);
	memset(&screen->utf8, 0, sizeof(screen->utf8));
	screen->state.cursor_x = 0;
	screen->state.cursor_y = 0;
	devcon_page_erase(screen->page_main, 0, 0, screen->page->width,
			  screen->page->height, &screen->state.attr,
			  screen->age, false);
	devcon_page_erase(screen->page_alt, 0, 0, screen->page->width,
			  screen->page->height, &screen->state.attr,
			  screen->age, false);
}
				 ...
				 const char *answerback)
{
	...
	if (answerback) {
		t = kstrdup(answerback, GFP_KERNEL);
		if (!t)
			return -ENOMEM;
	}
...
int devcon_screen_draw(struct devcon_screen *screen,
		       ...
		       u64 *fb_age)
{
	u64 cell_age, line_age, age = 0;
	struct devcon_charbuf ch_buf;
	const u32 *ch_str;
	unsigned int i, j, cw;
	struct devcon_page *page;
	...
	for (j = 0; j < page->height; ++j) {
		...
		for (i = 0; i < page->width; ++i) {
			...
			ch_str = devcon_char_resolve(cell->ch, &ch_n, &ch_buf);
			...
			attr = cell->attr;
			if (i == screen->state.cursor_x &&
			    j == screen->state.cursor_y &&
			    !(screen->flags & DEVCON_FLAG_HIDE_CURSOR))
				attr.inverse ^= 1;
